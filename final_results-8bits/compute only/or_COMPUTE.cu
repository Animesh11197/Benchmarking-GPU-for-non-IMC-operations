...
void *GPU_add_vectors(char *A, char *B, int N);
...
__global__ void add_vectors_kernel(char *A, char *B, int N) {
	...
	if (thread_id < N)  A[thread_id] || B[thread_id];
}


// Returns the vector sum A + B (computed on the GPU)
void *GPU_add_vectors(char *A_CPU, char *B_CPU, int N) {
	...
	add_vectors_kernel <<< grid_size , threads_per_block >>> (A_GPU, B_GPU, N);
  ...
  printf("\tTransfer from GPU: 0.00000 sec\n");	
