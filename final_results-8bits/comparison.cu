...
char *GPU_add_vectors(char *A, char *B, int N);
char *CPU_add_vectors(char *A, char *B, int N);
char *get_random_vector(int N);
...
__global__ void add_vectors_kernel(char *A, char *B, char *C, int N) {
	...
	if (thread_id < N) if (A[thread_id] < B[thread_id]) C[thread_id] = 0;
                     else if (A[thread_id] == B[thread_id]) C[thread_id] = 1;
                     else  C[thread_id] = 2; 
}


// Returns the vector sum A + B (computed on the GPU)
char *GPU_add_vectors(char *A_CPU, char *B_CPU, int N) {
	
	long long memory_start_time = start_timer();

	// Allocate GPU memory for the inputs and the result
	int vector_size = N * sizeof(char);
	char *A_GPU, *B_GPU, *C_GPU;
	...
	char *C_CPU = (char *) malloc(vector_size);
...
char *CPU_add_vectors(char *A, char *B, int N) {	
	...
	// Compute the sum;
	for (int i = 0; i < N; i++) if (A[i] < B[i]) C[i] = 0;
...
char *V = (char *) malloc(N * sizeof(char));
	...
	for (int i = 0; i < N; i++) V[i] = (char) rand() ;
