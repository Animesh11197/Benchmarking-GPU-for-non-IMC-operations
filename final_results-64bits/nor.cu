...
double *GPU_add_vectors(double *A, double *B, int N);
double *CPU_add_vectors(double *A, double *B, int N);
...
__global__ void add_vectors_kernel(double *A, double *B, double *C, int N) {
...
double *GPU_add_vectors(double *A_CPU, double *B_CPU, int N) {
	...
	double *C_CPU = (double *) malloc(vector_size);
...
double *V = (double *) malloc(N * sizeof(double));
	...
	for (int i = 0; i < N; i++) V[i] = (double) rand() ;
