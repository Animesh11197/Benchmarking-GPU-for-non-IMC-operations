//     EDITS : 1) Threads limited, lower limit.
//             2) long long data type
...
long long int *GPU_add_vectors(long long int *A, long long int *B, int N);
long long int *CPU_add_vectors(long long int *A, long long int *B, int N);
long long int *get_random_vector(int N);
...
__global__ void add_vectors_kernel(long long int *A, long long int *B, long long int *C, int N) {
...
long long int *GPU_add_vectors(long long int *A_CPU, long long int *B_CPU, int N) {
	
	long long memory_start_time = start_timer();

	// Allocate GPU memory for the inputs and the result
	int vector_size = N * sizeof(long long int);
	long long int *A_GPU, *B_GPU, *C_GPU;
	...
	long long int *C_CPU = (long long int *) malloc(vector_size);
...
long long int *CPU_add_vectors(long long int *A, long long int *B, int N) {	
	...
	long long int *C = (long long int *) malloc(N * sizeof(long long int));
...
long long int *V = (long long int *) malloc(N * sizeof(long long int));
	...
	for (int i = 0; i < N; i++) V[i] = (long long int) rand() ;
