...
long long *GPU_add_vectors(long long *A, long long *B, int N);
long long *CPU_add_vectors(long long *A, long long *B, int N);
long long *get_random_vector(int N);
...
__global__ void add_vectors_kernel(long long *A, long long *B, long long *C, int N) {
...
long long *GPU_add_vectors(long long *A_CPU, long long *B_CPU, int N) {
	
	long long memory_start_time = start_timer();

	// Allocate GPU memory for the inputs and the result
	int vector_size = N * sizeof(long long);
	long long *A_GPU, *B_GPU, *C_GPU;
	...
	long long *C_CPU = (long long *) malloc(vector_size);
...
long long *V = (long long *) malloc(N * sizeof(long long));
	...
	for (int i = 0; i < N; i++) V[i] = (long long) rand() ;
